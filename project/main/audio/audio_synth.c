...
static void sfx_tick(uint32_t elapsed_ms)
{
    ...
    if (elapsed_ms >= g_sfx.duration_samples * 1000 / AUDIO_SAMPLE_RATE) {
        /* SFX done — restore all channels */
        g_sfx.active = false;
        for (int i = 0; i < SYNTH_CH_COUNT; i++) {
            g_ch[i].volume = g_sfx.saved_vol[i];
            g_ch[i].active = true;
            /* Reset any SFX-used channel to silence so stale
             * midi_note values don't leak into BGM output */
...
void audio_synth_render(int16_t *buf, int num_samples)
{
    ...
    for (int n = 0; n < num_samples; n++) {
        ...
        if (s != 0) {
            ...
            else if (-s > max_abs) max_abs = -s;
        }

        g_sample_count++;
    }

    /* Diagnostic: log once per second whether synth is producing audio */
    static uint32_t last_diag = 0;
    static bool     did_dump   = false;
    if (g_sample_count - last_diag > (uint32_t)AUDIO_SAMPLE_RATE) {
        last_diag = g_sample_count;
        if (nz_count == 0) {
            ESP_LOGW(TAG, "DIAG: %d samples ALL ZERO — expected silence", num_samples);
        } else {
            ESP_LOGI(TAG, "DIAG: %d/%d non-zero samples, peak=%d (%.1f%% FS)",
                     nz_count, num_samples, (int)max_abs,
                     (double)max_abs * 100.0 / 32767.0);

            /* One-time dump of all channel states for debugging */
            if (!did_dump) {
                did_dump = true;
                for (int c = 0; c < SYNTH_CH_COUNT; c++) {
                    const char *chnames[] = {"P1","P2","TRI","NOISE"};
                    ESP_LOGE(TAG, "CH[%s]: active=%d note=%d freq=%.1f phase=%.3f "
                             "pinc=%.6f duty=%d vol=%d env=%.3f adsr_phase=%d "
                             "lfsr=0x%04X ns=%d",
                             chnames[c],
                             (int)g_ch[c].active, g_ch[c].midi_note,
                             (double)g_ch[c].frequency, (double)g_ch[c].phase,
                             (double)g_ch[c].phase_inc, g_ch[c].duty,
                             g_ch[c].volume, (double)g_ch[c].adsr.level,
                             (int)g_ch[c].adsr.phase,
                             g_ch[c].lfsr, (int)g_ch[c].noise_short);
                }
                ESP_LOGE(TAG, "SFX: active=%d type=%d dur=%lu/%lu",
                         (int)g_sfx.active, (int)g_sfx.type,
                         (unsigned long)(g_sample_count - g_sfx.start_sample),
                         (unsigned long)g_sfx.duration_samples);
                ESP_LOGE(TAG, "BGM: state=%d step=%d scnt=%lu/%lu",
                         (int)g_bgm.state, g_bgm.step,
                         (unsigned long)g_bgm.sample_counter,
                         (unsigned long)g_bgm.samples_per_step);
            }
        }
    }
}
