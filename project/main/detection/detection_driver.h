...
typedef struct {
    ...
    int         box[4];         /* 边界框 [x1, y1, x2, y2]，地图坐标 (640x640) */
    int         box_camera[4];  /* 边界框 [x1, y1, x2, y2]，相机坐标 (800x640) */
 ...
 * @param results          输出检测结果数组 (含地图坐标 box[] 和相机坐标 box_camera[])
 * @param count            输入/输出: 传入最大结果数, 传出实际结果数
 * @param preprocess_flags 预处理标志位组合 (PREPROC_FLAG_* 按位或), 0=不启用
 * @return ESP_OK 成功
 */
